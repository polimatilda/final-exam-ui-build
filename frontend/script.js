...
const bookComponent = (sub, title, text) => `
  <div class="book-card">
    <p class="book-id">id</p>
    <h3 class="book-author">${sub}</h3>
...
const loadEvent = () => {
  ...
  rootElement.insertAdjacentHTML('beforeend', `<div class="books-container"></div>`)

  const booksContainer = document.querySelector('.books-container')

  books.cards.map(book => {
    booksContainer.insertAdjacentHTML('beforeend', bookComponent(book.sub, book.title, book.text))
  })

  rootElement.insertAdjacentHTML('afterbegin', headerComponent(books.logo))

  const idCalculator = () => {
    const idClasses = document.querySelectorAll('.book-id')
